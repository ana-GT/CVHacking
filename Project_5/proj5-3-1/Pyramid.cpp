 ...
 * @function Expand
 */
cv::Mat Pyramid::ExpandFloat( const cv::Mat &_input ) {

    if( _input.type() != CV_32FC1 )
    { printf("--XX You distracted geek! Gimme a 32FC1 image. Argh! \n");}


   double minVal; double maxVal;
   cv::Point minPoint; cv::Point maxPoint;
   cv::minMaxLoc( _input, &minVal, &maxVal, &minPoint, &maxPoint, cv::Mat() );
   printf("Input: Min: %f -- Max: %f  Loc: Min: %d %d Max: %d %d \n", minVal, maxVal, minPoint.x, minPoint.y, maxPoint.x, maxPoint.y);
    ...
    float *expandedFloat;
    expandedFloat = new float[rows2*cols2];
    cv::Mat expanded = cv::Mat::zeros( rows2, cols2, CV_32FC1 );
    ...
    for( int j = 0; j < rows2; j++ ) {
     ...
     {  for( unsigned int i = 0; i < cols2; i++ )
        {
           expanded.at<float>(j,i) = expandedFloat[ind]/3.5; 
           ind++; 
        }
     }

   cv::minMaxLoc( expanded, &minVal, &maxVal, &minPoint, &maxPoint, cv::Mat() );
   printf("Expanded: Min: %f -- Max: %f  Loc: Min: %d %d Max: %d %d \n", minVal, maxVal, minPoint.x, minPoint.y, maxPoint.x, maxPoint.y);
